...
import { copilotToolCallIsSubagent, makeCopilotAdapter } from "./CopilotAdapter.ts";
...
it("only classifies task agent types as Copilot subagents", () => {
  const toolCall = (rawInput: Record<string, unknown>, title?: string) =>
    ({
  ...
  assert.isFalse(copilotToolCallIsSubagent(toolCall({ agent_type: "shell" })));
  assert.isTrue(copilotToolCallIsSubagent(toolCall({ agent_type: "shell", agent: "researcher" })));
  assert.isTrue(copilotToolCallIsSubagent(toolCall({ agent_type: "task" })));
  assert.isTrue(copilotToolCallIsSubagent(toolCall({ agent: "researcher" })));
  assert.isTrue(copilotToolCallIsSubagent(toolCall({}, "task: researcher")));
});
