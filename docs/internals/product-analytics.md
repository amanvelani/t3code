...
T3 Code can send anonymous product events from the server to PostHog when a
user opts in through Settings -> General -> Share usage data. The
setting defaults to off. Until it is enabled, the server does not read or
create a telemetry identifier, buffer events, or make PostHog requests. When
enabled, the server uses the first available hashed Codex account ID, hashed
Claude user ID, or installation-scoped anonymous ID as the distinct ID.
Clients do not load the PostHog browser SDK.
